...
import { checkinAction } from '../../store/Slices/checkInSlice';
...
function StudentPage() {
  ...
  const checkIn = ()=>{
    const {_id, course_name, roll_no, picture ,name} = JSON.parse(localStorage.getItem("authUser"))
    
    dispatch(checkinAction({_id, course_name, roll_no, picture ,name}))
  }
  useEffect(() => {
    const { name, picture, course_name, roll_no } = JSON.parse(localStorage.getItem("authUser"))
  ...
  return (
    <div className="flex  flex-col md:flex-row h-screen">
      {/* Left Sidebar */}
      <div className="w-full bg-gray-800 text-white md:w-1/5 text-black p-6 flex flex-col justify-between md:sticky top-0 h-screen bg-white border-r">
        <div>
          <div className="mb-8">
        ...
        <div className="mt-auto cursor-pointer" onClick={logout}>Logout</div>
      </div>

     
      <div className="w-full bg-gray-600 text-white md:w-4/5 p-6 bg-gray-100 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200">
        ...
        {currentPage === 'My Attendance' && (
          <div className="mb-6 max-h-96 overflow-y-auto relative">
            ...
            <div className="flex justify-end mb-4 sticky top-0 z-10">
              <button className="bg-blue-500 text-white rounded-full p-2 mr-2" onClick={checkIn}>Check In</button>
            </div>
